...
use Qodehub\Bitgo\Wallet\SendTransaction;
...
class SendTransactionTest extends TestCase
{
    ...
    {
        $instance = SendTransaction::{$this->coin}();

        $this->assertInstanceOf(SendTransaction::class, $instance);

        $this->assertEquals($instance->getCoinType(), $this->coin);
    }

    /** @test */
    public function it_should_throw_and_error_if_both_the_txHex_and_halfSigned_are_absent()
    {
        /**
         * Mock the getClient method in the SendTransaction to intercept calls to the server
         */
        $mock = $this->getMockBuilder(SendTransaction::class)
    ...
    public function test_that_a_call_to_the_server_will_be_successful_if_all_is_right()
    {
        ...
        /**
         * Listen to the SendTransaction class method and use the interceptor
         *
        ...
        $this->assertContains(
            "https://some-host.com/api/v2/" . $this->coin . "/wallet/" . $this->walletId . '/tx/send',
